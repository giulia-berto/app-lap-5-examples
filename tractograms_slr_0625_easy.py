...
def tractograms_slr(mov_id, moving_tractogram, stat_id, static_tractogram):
	...
	affine = srm.matrix
	print('%s' %affine)
	np.save('m%s_s%s_affine' %(mov_id, stat_id), affine)
...
if __name__ == '__main__':
	...
	parser.add_argument('-mov_id', nargs='?', const=1, default='',
	                    help='The moving subject filename')
	...
	parser.add_argument('-stat_id', nargs='?',  const=1, default='',
	                    help='The static subject filename')
	parser.add_argument('-static', nargs='?',  const=1, default='',
	                    help='The static tractogram filename')                   
	args = parser.parse_args()

	tractograms_slr(args.mov_id, args.moving, args.stat_id, args.static)	
